...
def render_dashboard(device_info, scan_results, current_task=None):
    ...
    layout.split(
        ...
        Layout(Panel(table, title="[bold white]Real-Time Security Audit[/]", border_style="cyan"), ratio=1),
...
class DashboardManager:
    ...
    def update(self, module_name, status, summary):
        ...
        if self.live:
            self.live.update(render_dashboard(self.device_info, self.scan_results, self.current_task))
    ...
    def start(self):
        self.live = Live(render_dashboard(self.device_info, self.scan_results, self.current_task), refresh_per_second=4)
        self.live.start()
